...
const Computers = () => {
    ...
    const [Computers_list, setComputers_list] = useState([]);
    ...
    async function getComputers() {
        const apiData = await API.graphql({ query: listForSales, variables: { filter: { type: { eq: "computers" } } } });
        const listData = apiData.data.listForSales.items;
        
        setComputers_list(listData);
    }
    ...
    async function deleteItem({ id, title }) {
        const newHousing = Computers_list.filter((house) => house.id !== id);
        setComputers_list(newHousing);
    ...
    return (
        <div>
            ...
            <div className="contentContainer">
                <h1 className="contentTitle">Computer Postings</h1>
                {Computers_list.map((estate) => (
