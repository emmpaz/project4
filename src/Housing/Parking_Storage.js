...
const ParkingStorage = () => {
    ...
    const [ParkingStorage_list, setParkingStorage_list] = useState([]);
    ...
    async function getParkingStorage() {
        const apiData = await API.graphql({ query: listHousings, variables: { filter: { type: { eq: "parkingstorage" } } } });
        const listData = apiData.data.listHousings.items;
        
        setParkingStorage_list(listData);
    }
    ...
    async function deleteHouse({ id, title }) {
        const newHousing = ParkingStorage_list.filter((house) => house.id !== id);
        setParkingStorage_list(newHousing);
    ...
    return (
        <div>
            ...
            <div className="contentContainer">
                <h1 className="contentTitle">Parking/Storage Listings</h1>
                {ParkingStorage_list.map((estate) => (
