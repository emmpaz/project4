...
const Security = () => {
    ...
    const [Security_list, setSecurity_list] = useState([]);
    ...
    async function getSecurity() {
        const apiData = await API.graphql({ query: listJobs, variables: { filter: { type: { eq: "security" } } } });
    ...
    async function deleteJob({ id, title }) {
        const newHousing = Security_list.filter((house) => house.id !== id);
        setSecurity_list(newHousing);
