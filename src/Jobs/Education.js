...
const Education = () => {
    ...
    async function getEducation() {
        const apiData = await API.graphql({ query: listJobs, variables: { filter: { type: { eq: "education" } } } });
    ...
    async function deleteJob({ id, title }) {
        const newHousing = Education_list.filter((house) => house.id !== id);
        setEducation_list(newHousing);
