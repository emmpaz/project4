...
const Activities = () => {
    ...
    const [Activities_list, setActivities_list] = useState([]);
    ...
    async function getActivities() {
        const apiData = await API.graphql({ query: listCommunities, variables: { filter: { type: { eq: "activities" } } } });
    ...
    async function deleteCommunity({ id, title }) {
        const newHousing = Activities_list.filter((house) => house.id !== id);
        setActivities_list(newHousing);
