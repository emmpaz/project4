...
const Volunteers = () => {
    ...
    const [Volunteers_list, setVolunteers_list] = useState([]);
    ...
    async function getVolunteers() {
        const apiData = await API.graphql({ query: listCommunities, variables: { filter: { type: { eq: "volunteers" } } } });
        const listData = apiData.data.listCommunities.items;
        
        setVolunteers_list(listData);
    }
    ...
    async function deleteCommunity({ id, title }) {
        const newHousing = Volunteers_list.filter((house) => house.id !== id);
        setVolunteers_list(newHousing);
    ...
    return (
        <div>
            ...
            <div className="contentContainer">
                <h1 className="contentTitle">Volunteer Postings</h1>
                {Volunteers_list.map((estate) => (
